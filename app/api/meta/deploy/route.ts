...
export async function DELETE() {
  ...
  try {
    ...
    await db.from("users").update({ metaapi_account_state: "UNDEPLOYED" }).eq("id", session.user.id);
    return NextResponse.json({ success: true });
  } catch (e) {
    return NextResponse.json({ error: e instanceof Error ? e.message : "Undeploy failed" }, { status: 502 });
  }
}
