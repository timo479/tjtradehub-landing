...
const SESSIONS = ["London", "New York", "Asien", "London/NY Overlap", "NY/Asien Overlap"];
...
export default function TradeForm({ onClose, onSaved, trade }: Props) {
  ...
  const riskWarning = (() => {
    ...
    if (riskSettings.max_risk_per_trade) {
      ...
      if (actual > max * 1.2) {
        return `⚠️ Risiko-Limit überschritten! Du riskierst ${actual.toFixed(2)} CHF – dein Limit liegt bei ${max.toFixed(2)} CHF (+${(((actual - max) / max) * 100).toFixed(0)}% über deinem Plan).`;
      }
    }
    return null;
  })();
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
            ...
            <div>
              ...
              <input type="text" style={inputStyle} placeholder="z.B. XAUUSD" value={form.symbol} onChange={e => set("symbol", e.target.value)} required />
            </div>
          </div>

          {/* Richtung – immer sichtbar */}
          <div>
          ...
          {modules.strategy && strategies.length > 0 && (
            <div style={{ backgroundColor: "#0d1117", border: "1px solid #1F2937", borderRadius: "12px", padding: "16px" }}>
              <label style={{ ...labelStyle, marginBottom: "10px" }}>Strategie</label>
              ...
              {selectedStrategy && (
                <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "4px" }}>
                    <p style={{ color: "#6B7280", fontSize: "12px" }}>Regelcheckliste</p>
                    {autoScore !== null && (
                      <span style={{
          ...
          {modules.position && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px" }}>
              ...
              <div>
                ...
                <input type="number" step="any" style={inputStyle} placeholder="z.B. 50" value={form.risk_amount} onChange={e => set("risk_amount", e.target.value)} />
              </div>
              <div>
                <label style={labelStyle}>P&L (CHF) *</label>
                <input type="number" step="any" style={inputStyle} placeholder="-50 oder +120" value={form.pnl} onChange={e => set("pnl", e.target.value)} required />
              </div>
            </div>
          )}
          ...
          {modules.custom && customFields.length > 0 && (
            <div style={{ backgroundColor: "#0d1117", border: "1px solid #1F2937", borderRadius: "12px", padding: "16px" }}>
              <p style={{ color: "#6B7280", fontSize: "12px", marginBottom: "12px" }}>Eigene Felder</p>
          ...
          <div style={{ display: "flex", gap: "12px", marginTop: "8px" }}>
            ...
            <button type="submit" disabled={loading} style={{
              ...
              {loading ? "Speichern..." : riskWarning ? "⚠️ Trotzdem speichern" : trade ? "Änderungen speichern" : "Trade speichern"}
            </button>
          </div>
        </form>
      </div>
